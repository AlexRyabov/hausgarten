...
%    atomatic location of letters within the axis boxes but it is much slower because in this case
%    drawnow is called. It is fine to use this option at the end of calculation, 
%    but not if the function is called many times within a long loop 
%    (which is anyway not a good idea)
%    
%    'Direction' = 'LeftRight' or 'TopDown'  label figures either from left
%    to right or from top to down
%
% Example:
%    AddLetters2Plots  adds  A, B, ...  to the current figure
...
%      (b) and (c) to axes defined by the handles ax1, ax2, ax3
%
%    AddLetters2Plots(fg, 'HShift', -0.08, 'VShift', -0.02, 'Direction', 'TopDown')
%       ads Default letters outside of figures, the order the from top to
%       bottom
%
% Code example
% add letters to all axes from left to right and then from top to bottom
% fg = figure(1);
% clf
% subplot(2, 2, 1)
% subplot(2, 2, 2)
...
% %% 
% %% Add red letters in the bottom left corner
% figure(3);
% ax1 = subplot(2, 2, 1);
% ax2 = subplot(2, 2, 2);
% ax3 = subplot(2, 1, 2);
...
% Website:
% July 2021; Last revision: Feb 2022
arguments
    Fg_or_axes = gcf;
...
if isa(Fg_or_axes, 'matlab.ui.Figure')
    ...
    for iCh = 1:length(Children)
        if isa(Children(iCh), 'matlab.graphics.axis.Axes') || ...
                ...
                isa(Children(iCh), 'matlab.graphics.chart.HeatmapChart') ||...
...
%Define the horizontal and vertical shift
...
%Delete labels if any were added
Ind = findall(Parent,'UserData','ARTextLabel');
if ~isempty(Ind)
    delete(Ind);
end
...
for ia = 1:length(lAxes)
    ...
    h  = annotation(Parent, 'textbox',...
        ...
        'Color', NV.Color, ...
        'FitBoxToText','on');
    h.UserData = 'ARTextLabel';
    if NV.FitLocation  % a bit slow!! becase of drawnow function, otherwise h.Position(3) == 0 :-(, 
        %how to get the textbox size without refreshing everything???
        %It is fine to use at the end of calcualtion, but not in a loop 
        drawnow
        Pos = h.Position;
        switch NV.Location
            case 'NorthWest' %(default)
